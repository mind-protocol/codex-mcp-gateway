...
export type LaunchCodexTaskInput = z.infer<typeof LaunchCodexTaskInput>;

export interface LaunchCodexTaskResult {
  run_id: string;
  audit_url: string;
}

export async function launchCodexTask(
  input: LaunchCodexTaskInput,
  github: GitHubClient,
  events: EventBus
): Promise<LaunchCodexTaskResult> {
